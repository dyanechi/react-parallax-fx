...
export const animateFilter = (filters: FilterProps, progress: number) => {
  const {
    blur,
    brightness,
    contrast,
  ...
  if (dropShadow)
    _filterStr += `drop-shadow(${lerp(...dropShadow, progress)}px) `;
